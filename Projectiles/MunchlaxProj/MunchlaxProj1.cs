...
{
    public class MunchlaxProj1 : ModProjectile
    {
        ...
        private int attackShooting_1 = 0;// 撞击
        private int attackShooting_2 = 0;// 逐步击破
        private int attackShooting_3 = 0;// 变圆
        private int attackShooting_4 = 0;// 泰山压顶
        ...
        void AttackShooting_1(NPC target)
        {
            ...
            if (attackShooting_1 == (munchlaxBadge.level >= 0 && munchlaxBadge.level < 6 ? 240 : (munchlaxBadge.level >= 6 && munchlaxBadge.level < 9 ? 180 : 120))) // 攻击间隔为120帧
            {
                attackShooting_1 = 0; // 重置计时器
                    //生成新的弹幕
                    Projectile.NewProjectile(Projectile.GetSource_FromAI(),
                        Projectile.Center, (target.Center - Projectile.Center).SafeNormalize(Vector2.Zero) * 26f,
                        ModContent.ProjectileType<MunchlaxProj2>(), // 生成我们自己写的弹幕
                        Projectile.originalDamage + (int)(player.GetWeaponDamage(player.inventory[player.selectedItem]) * 2f), Projectile.knockBack, Projectile.owner, // 为接下来生成的弹幕提供主人
        ...
        void AttackShooting_2(NPC target)
        {
            ...
            if (attackShooting_2 == (munchlaxBadge.level >= 6 && munchlaxBadge.level < 9 ? 180 : 120)) // 攻击间隔为120帧
            {
                attackShooting_2 = 0; // 重置计时器
                //生成新的弹幕
                Projectile.NewProjectile(Projectile.GetSource_FromAI(),
                    target.Center, (target.Center - Projectile.Center).SafeNormalize(Vector2.Zero) * 0f,
                    ModContent.ProjectileType<MunchlaxProj3>(), // 生成我们自己写的弹幕
        ...
        void AttackShooting_3(NPC target)
        {
            ...
            if (attackShooting_3 == 120) // 攻击间隔为120帧
            {
                ...
                //生成新的弹幕
                Projectile.NewProjectile(Projectile.GetSource_FromAI(),
                    player.Center, (target.Center - Projectile.Center).SafeNormalize(Vector2.Zero) * 0f,
                    ModContent.ProjectileType<MunchlaxProj4>(), // 生成我们自己写的弹幕
                    Projectile.originalDamage + (int)(player.GetWeaponDamage(player.inventory[player.selectedItem]) * 0.2f), Projectile.knockBack, Projectile.owner, // 为接下来生成的弹幕提供主人
                    target.whoAmI); // 传入敌人的号码，为接下来生成的弹幕提供目标
            }
        }
        ...
        void AttackShooting_4(NPC target)
        {
            ...
            if (attackShooting_4 == 120) // 攻击间隔为120帧
            {
                ...
                //生成新的弹幕
                Projectile.NewProjectile(Projectile.GetSource_FromAI(),
                    target.Center + new Vector2(0, -600), (target.Center - Projectile.Center).SafeNormalize(Vector2.Zero) * 0f,
                    ModContent.ProjectileType<MunchlaxProj5>(), // 生成我们自己写的弹幕
                    Projectile.originalDamage + (int)(player.GetWeaponDamage(player.inventory[player.selectedItem]) * 2f), Projectile.knockBack, Projectile.owner, // 为接下来生成的弹幕提供主人
                    target.whoAmI); // 传入敌人的号码，为接下来生成的弹幕提供目标
            }
        }
        ...
        public override void AI()
        {
            ...
            if (player.HasBuff<BuffsMunchlaxBadge>())
                Projectile.timeLeft = 2;

            Projectile.damage = 0;
            bool isFindProj2 = false;
            for (int i = 0; i < Main.maxProjectiles; i++)
            {
                Projectile proj = Main.projectile[i];
                if (proj.active && proj.owner == Projectile.owner && proj.type == ModContent.ProjectileType<MunchlaxProj2>())
                {
                    isFindProj2 = true;
                    break;
                }
            ...
            if (isFindProj2)
            {
                ...
                Projectile.light = 0; // 取消发光
            }
            else
            {
                Projectile.alpha = 1; // 透明度恢复
                Projectile.light = 0.5f; // 发光亮度恢复
            }
            // 定义目标NPC
            NPC target = null;
            ...
            if (target != null && target.active) // 如果目标不为空且存活在此处执行攻击性AI
            {
                    ...
                    if (pokeRadar != null)
                    {
                        ...
                        for (int j = 0; j < PokeRadar.MaxItems; j++)
                        {
                            if (pokeRadar.items[j] != null && !pokeRadar.items[j].IsAir)
                                if (pokeRadar.items[j].type == ModContent.ItemType<MunchlaxBadge>())
                                {
                                    if (pokeRadar.items[j].ModItem is MunchlaxBadge munchlax)
                                    {
                        ...
                        if (equippedIndex != -1)
                        {
                            ...
                            for (int i = 0; i < equippedIndex; i++)
                            {
                                if (pokeRadar.items[i] != null && !pokeRadar.items[i].IsAir)
                                {
                                    ...
                                    if (pokeRadar.items[i].type == ModContent.ItemType<MunchlaxBadge>())
                                        break;
                                }
                            }
                ...
                {
                    if (lastLevel != munchlaxBadge.level)
                    {
                        // 等级发生变化，重置所有攻击相关状态
                        attackType = 0;
                        attackTime = 0;
                        ...
                        standtime_1 = 0;
                        // 其他需要重置的状态也可以加上
                        lastLevel = munchlaxBadge.level;
                    }
                }
                if (munchlaxBadge != null && munchlaxBadge.level >= 0 && munchlaxBadge.level < 6)
                {
                    AttackShooting_1(target);// 撞击
                }
                else if (munchlaxBadge != null && munchlaxBadge.level >= 6 && munchlaxBadge.level < 9)
                {
                    switch (attackType)
                    {
                        case 0:
                ...
                else if (munchlaxBadge != null && munchlaxBadge.level >= 9 && munchlaxBadge.level < 12)
                {
                    switch (attackType)
                    {
                        case 0:
                ...
                else if (munchlaxBadge != null && munchlaxBadge.level >= 12)
                {
                    switch (attackType)
                    {
                        case 0:
                        ...
                        case 2:
                            ...
                            if (attackTime == 120)
                            {
                                if (Math.Abs(target.Center.X - player.Center.X) > 100)
                                    attackType = 3;
                                else
                                    attackType = 0;
                                attackTime = 0;
            ...
            else if(Projectile.ai[1] > 60 && isstand && 
                (Projectile.frame != 9 || Projectile.frame != 10 || Projectile.frame != 11))
            {
                Projectile.ai[0] += 1;
                Projectile.ai[1] = 0;
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (Projectile.velocity.Length() > 9 || returnToPlayer)
                for (int i = 0; i < ProjectileID.Sets.TrailCacheLength[Type]; i++)
                {
                    ...
                    Main.EntitySpriteDraw(texture, oldcenter, rectangle, MyColor * factor * Projectile.Opacity,
                        Projectile.oldRot[i],
        ...
        private static bool CanSeeThroughTwoWalls(Vector2 playerCenter, Vector2 npcCenter)
        {
            ...
            for (int i = 0; i < Main.maxProjectiles; i++)
            {
                ...
                if (projectile.active && projectile.owner == Main.myPlayer && 
                    projectile.type == ModContent.ProjectileType<MunchlaxProj1>())
                {
