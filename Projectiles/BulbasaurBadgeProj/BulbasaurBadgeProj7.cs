...
{
    public class BulbasaurBadgeProj7 : ModProjectile
    {
        ...
        public override void AI()
        {
            if(Projectile.timeLeft <=330)
            {
                ...
                if (target != null && target.active && target.damage > 0)
                {
                    ...
                    // 向目标方向移动
                    Projectile.velocity = vectorToTarget * 4f + Projectile.velocity * 0.6f; // 速度为10

                    //加速度
                    Projectile.velocity.X += Main.rand.Next(-10, 11) * 0.1f;
                    Projectile.velocity.Y += Main.rand.Next(-10, 11) * 0.1f;
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                int dust = Dust.NewDust(Projectile.Center + Projectile.velocity, Projectile.width + Main.rand.Next(-40, 40),
                    Projectile.height + Main.rand.Next(-40, 40), DustID.WhiteTorch, Projectile.velocity.X * 0.2f, Projectile.velocity.Y * 0.2f);
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            Projectile.NewProjectile(Projectile.GetSource_FromAI(),
                ...
                ModContent.ProjectileType<BulbasaurBadgeProj8>(),
